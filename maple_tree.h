...
// Nodes are 256 bytes in size
// Slots represent the branching factor/maximum number of possible children a node can have


// These are slot definitions not nodes
#if defined(CONFIG_64BIT) || defined(BUILD_VDSO32_64)
/* 64bit sizes */
#define MAPLE_NODE_SLOTS	31	/* 256 bytes including ->parent */    // These are meant for generic nodes and are used for allocation purposes
#define MAPLE_RANGE64_SLOTS	16	/* 256 bytes */         // These are meant for the nodes that i have been drawing that have a branching factor of 16
#define MAPLE_ARANGE64_SLOTS	10	/* 240 bytes */    // These are allocation tree nodes which trackgaps and have a branching factor of 10
...
struct maple_arange_64 {
	...
	unsigned long gap[MAPLE_ARANGE64_SLOTS];  // Records the largest NULL range(ie gaps) available in each subtree
...
// Maple Tree is optimized to store one entry directly in the root without allocating a full node. Saves memory and improves cache locality for very small trees.
...
// Initialize a maple tree with an internal lock ie ma_lock
 ...
 * low bits for our own purposes.
 *
 * Nodes are currently of 4 types:
 * 1. Single pointer (Range is 0-0)
 * 2. Non-leaf Allocation Range nodes (maple_arange_64)
 * 3. Non-leaf Range nodes (maple_range_64)
 * 4. Leaf Range nodes (maple_leaf_64)
 *
