...
static inline unsigned long mte_parent_slot_mask(unsigned long parent)
{
	...
	// basically checks if the second bit(ie bit 1) is set or not
	// If bit 1 is set (1) → parent uses MAPLE_PARENT_SLOT_MASK
	// If bit 1 is clear (0) → parent uses MAPLE_PARENT_16B_SLOT_MASK
	if (likely(parent & MAPLE_PARENT_NOT_RANGE16))
...
static inline void *mtree_range_walk(struct ma_state *mas)
{
	...
	do {
		...
		if (pivots[0] >= mas->index) {
			...
			max = pivots[0];
			// min stays unchanged since slot 0 inherits the current min
...
next:
		...
		if (unlikely(ma_dead_node(node)))
			goto dead_node;
	} while (!ma_is_leaf(type));  // Continue until we get to a leaf node ie the bottom of the tree 
